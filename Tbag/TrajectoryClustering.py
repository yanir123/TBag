...
class TBAG(object):
    ...
    def __init__(self,
        ...
        time_eps : float, optional
            maximum time difference when the first cluster ends and the second starts, by default np.inf
    ...
    def fit(self,
        ...
        cast_to_radians : bool, optional
            condition that indicates if the data should be cast to radians, by default False
        """
    ...
    def check_params(data: pd.DataFrame, lat_col: str, lon_col: str,
        ...
        `data` : np.ndarray | pd.DataFrame
            ...
            if `pd.DataFrame` the *_col parameters represent the keys that hold the relevant data.
        `lat_col` : str
            column name of the latitude value
        `lon_col` : str
